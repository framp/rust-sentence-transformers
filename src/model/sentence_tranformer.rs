...
impl SentenceTransformer {
    ...
    pub fn encode(&self, sentences: Vec<&str>, batch_size: Option<usize>) -> Vec<Vec<f64>> {
        let batch_size = if let Some(value) = batch_size {
            value
        } else {
            8
        };
        let mut all_embeddings: Vec<Vec<f64>> = vec![];
        let length_sorted_idx = argsort(
            sentences
                .iter()
                .map(|&input| input.len())
                .collect::<Vec<_>>()
        ...
        for batch_idx in (0..sentences.len()).step_by(batch_size) {
            ...
            for text in &batch_tokens {
                let (input_ids, token_type_ids, input_mask, _sentence_length) =
                    self.bert.get_sentence_features(&text, longest_seq);
                input_ids_feature.push(Tensor::from_slice(&input_ids));
                token_type_ids_feature.push(Tensor::from_slice(&token_type_ids));
                input_mask_feature.push(Tensor::from_slice(&input_mask));
            }
            ...
            all_embeddings.extend(from::<Vec<Vec<f64>>>(&features.sentence_embedding.unwrap()));
        }

        let reverting_order = argsort(&length_sorted_idx);

        let all_embeddings = reverting_order
            .iter()
